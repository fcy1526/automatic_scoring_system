<template>
  <div>
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>学生表现评分</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 卡片视图区域 -->
    <el-card>
      <el-row :gutter="20">
    ...
    <!-- 学生表现评分的对话框 -->
    <el-dialog title="学生表现评分" :visible.sync="scoreDialogVisible" width="40%"
...
export default {
  ...
  methods: {
    // 查询课程
    async getCourseList () {
      const { data: res } = await this.$http.get('scoreapi/course/process', { params: this.queryInfo })
    ...
    async getStudentList () {
      const { data: res } = await this.$http.get('scoreapi/score/student', { params: this.scoreClassInfo })
      if (!res.returnCode) return this.$message.error(res.returnMsg)
    ...
    // 保存评分
    async saveScore () {
      const { data: res } = await this.$http.post('scoreapi/score/student',
        { scoreList: this.studentList, currentStage: this.currentStage, courseId: this.scoreClassInfo.courseId })
      if (!res.returnCode) return this.$message.error(res.returnMsg)
